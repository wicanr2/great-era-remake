...
const (
	...
	//
	// ⭐ `sub_3C0D5`（`docs/re/31` §23）**對 3 與 4 一視同仁**：
	// 兩者都會被派去打選中的城市（寫 `+10` 目標單位、`+12` 下一跳）。
	// 所以功能上 3 與 4 同屬「**等待指派目標**」，差別仍未解。
	BattleCmdCommitted uint8 = 4
	// BattleCmdUnknown5 是 **5：已經朝某個城市去了**。
	//
	// `sub_3B492` 用「`+12` 正好是這個城市格 **且** `+9 == 5`」把城市
	// 從候選清單剔除（`battlecity.go`）——有人去了就別再派人。
	// ⚠️ 全檔有 **6 支**函式比較命令 5（`docs/re/31` §36 的表），
	// 但 **0 支**直接寫立即數 5——值從哪裡來仍未解。
	//
	// ⚠️ **值從哪裡來仍未解**：沒有任何函式寫立即數 5 進 `+9`。
	// 可能來自存檔，或來自某個計算結果。
