...
//
// ⛔ 這裡原本寫「哪個號碼是哪個兵種**沒有證據**」。**那是過時的**——
// 同一個 package 的 `general.go` 早就用 `MAN(N).DAT` 的最大值定了名
// （`docs/spec/02` §4），而 `sub_18A67`（`docs/re/32` §7）更把四個滿員數
// 直接寫在 `cmp` 裡，是程式碼側的第二個獨立來源：
//
//	兵種 1 → 20,000  步兵
//	兵種 6 → 10,000  騎兵
//	兵種 4 →  2,000  砲兵
//	兵種 5 →    200  裝甲
//
// ⚠️ **權重與滿員數不成反比**。三個吻合（1×20000、10×2000、100×200
// 都是 20,000），但騎兵是 4×10000 = 40,000——**騎兵是例外**。
// 不要用「滿員 = 20000 ÷ 權重」去推任何東西。
...
type StrengthInput struct {
	...
	Force   uint16 // +17，兵力
	// F19 是 **士兵戰技**（`docs/re/32` §9）。兩個獨立來源：
	// 練兵指令 `sub_1C916` 加的就是這一格，而那一支印的畫面文字是
	// 「司令欲練兵嗎？**士兵戰技**提升了！」（`docs/re/27` §11）。
	//
	// 徵新兵會讓它 **−20%**（`sub_18F5B`）——新兵拉低整體戰技。
	F19 uint8 // +19，士兵戰技
	// F20 語意未解。已知的行為：徵新兵讓它 **−25%**，而且有一道
	// 「`+20 ≥ 20` 才扣」的門檻（`sub_18F5B`）。
	//
	// ⚠️ 將領狀態畫面列的屬性裡「**武裝程度**」是最合理的候選
	// （新兵沒裝備），但**沒有直接證據**——要找到「加 `+20`」的地方
	// 才能定名。**不要拿這個假說當結論。**
	F20 uint8 // +20，語意未解（疑似武裝程度）
...
//
// **F 是發動攻擊的一方**（`docs/re/09` §2），所以這條規則的意思是
// **遠程攻擊者不吃反擊**——兵種 4 是遠程兵種，有攻擊範圍檢查、
// 森林不能發動、守方彈藥不足會放棄。
//
