...
**自傳資料庫**：名冊 417 種寫法（實際 **415 人**，兩組是異體字重複），
骨架已建 **393**，成文 300 篇上下。全部走「事實骨架與成文分離」的著作權隔離流程。

---

## 2. ⚠️ 正在跑的事——接手第一件事是確認它們

> **2026-08-02 決定：D1／D2／D3 做完就暫停自傳程式，不再派新批次。**
> 原因是 token 預算，不是遇到障礙——**這批交件驗收完就是乾淨的停點**。
> 名冊 417 筆屆時會剩約 24 筆未做（`--cross` 會列出是哪些）。
> 續做的話直接照 §3.2，不需要重新盤點。

交接當下有**三支 subagent 仍在寫檔**：`batchD1`／`batchD2`／`batchD3`
（自傳骨架與成文，共 151 人）。

```sh
git status --short        # 若還看得到未追蹤的 facts-batchD*.json，代表沒跑完
```

**在確認它們結束之前，主迴圈不要 `git add -A`、不要 `git add .`。**
2026-08-01 就是這樣把一支 agent 的**未完成檔**掃進 commit 的
（`CLAUDE.md` §10 有完整事故紀錄）。要 commit 就逐一列路徑。

跑完之後照這個順序驗收：

1. `./tools/py.sh tools/bio_gate.py batchD1`（D2、D3 各一次）——四項要全過，
   **自己重跑，不採信 agent 的回報**（規格 §7.3）
2. `./tools/py.sh tools/bio_gate.py --cross`——查跨批次重複
3. 把 agent 回報的 `G-D*-n`（規格沒涵蓋到的情形）寫成
   `docs/design/21-biography-authoring-spec.md` 的 §27
...
- **名冊最後約 24 筆**（`./tools/py.sh tools/bio_gate.py --cross` 會列出）
