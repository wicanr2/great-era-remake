...
> [`CLAUDE.md`](./CLAUDE.md)。
>
> 最後更新：2026-08-01（M0 完成、省份資料表全解、`TERNAME` 訂正為地名索引）
...
| **地圖／地名（M3）** | ✅ **SPEC-01 READY**：39 省鄰接表（對稱性 + 實機雙重驗證）、每省 14×14 戰場格子、**地名索引 39/39 驗證**。地名→地形圖塊的對應未解 |
| **將領（M3）** | ✅ **SPEC-02 READY**：33 B/人、三個能力值、兵力 u16@17、**省兵力 = 將領加總**（實機 97500 零誤差）|
| **Go 規則層（M4）** | 🔶 `internal/game`：地圖（SPEC-01）+ 將領（SPEC-02）+ **省份（SPEC-03）**，全部 byte-for-byte round-trip |
| **Ebiten 呈現層（M5）** | 🔶 **可互動**：`cmd/dsds` 用原版資料畫 39 省戰場格子 + 省名，←→ 切換省份。合成層不依賴 Ebiten，**640×350 全畫面 + 字模繪製都逐像素通過** |
...
| 1a | **地名 → 地形圖塊** | `TERNAME` 已確定是地名索引，但「長江該畫哪一張 `NEWTERR` 圖塊」的對照表未找到。`NWMAP.DAT` 是候選；或從 `sub_4EFBE` 往下追 |
...
| 3 | `MAN(N).DAT` 欄位 | 33 B/人，`+0/+1/+2` 已知是能力值、`+17` 兵力。**將領 ID 是 1-based 指向 `MAN{期}15` 名表**（SPEC-03 驗證）。其餘 28 B 待解 |
...
| 9 | `RAIL.TPC` | 8,064 B，檔頭解出 1×1，不是 BGI 也不是圖集，格式待查 |
