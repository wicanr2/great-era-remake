...
| 存檔 `.DT1`/`.DT2` | 🔶 **`.DT1` = 省份資源**（黃金/糧食/彈藥/煤礦/燃料/鐵礦，offset 929 起連續 u16，已用實機數值定名）。`.DT2` = 39×469，語意未解 |
...
| **政略規則（M3）** | 🔶 **起步**：15 個指令、命令數上限、省編號 = `3.15` 索引+1、13 個省份欄位。見 `docs/playtest/02` |
| Go 規則層／Ebiten | ⬜ 未開始（M4／M5，要等 M3 規格）|
...
| # | 項目 | 說明 |
|---|---|---|
| 1 | **`.DT1` 記錄間隔** | 資源欄位已定名（offset 929 起 6 個 u16），但 14,683 不整除 39。切換省份拿第二組數值即可定出間隔 |
| 1b | 15 個政略指令 | 逐一試，對照 `2.15` 的 `調動/軍事/運補/徵稅/徵兵…`（16 個詞 vs 15 個指令，**數量對不上，別對號入座**）|
