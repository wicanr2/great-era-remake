...
| **移動／機動力（M3）** | 🔶 **骨架 confirmed**：`+6`=機動力上限、`+7`=剩餘，移動一格 `+7 -= 成本`；三道 gate（相鄰、可進入、機動力夠）。**成本公式與 `sub_4A583` 的可進入規則未解** |
...
| 1j | `byte_91E[196]` | 每格一個 byte，觀察到 `50h`／`24h`／`0Dh`／`0FFh`。像是狀態或成本表，**沒有證據前不猜** |
