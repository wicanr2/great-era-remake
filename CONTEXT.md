...
| 1g | 執行期單位記錄 | 基址 `0x7A7D`、33 B/筆。已知 `+5`(0xFF=空槽)、`+16`(bit 0)、`+30`(衰減)。**與 `MAN(N).DAT` 的欄位對不上**，是兩套結構 |
...
| 1c | **`.DT2` 的部隊欄位** | 469 B 結構已解、`+28/+48` 已解出是攻守參戰單位（`docs/re/05`），但兩個 200 B 詳細區的語意未解。**要打進實際戰鬥取 `.DT2`**，而那條路目前被按鍵問題擋住（`docs/playtest/03`）|
...
| 6 | 戰鬥規則 | 機動力、地形防禦（社群說法，待驗）。⚠️ **兩個矛盾點**：(1) 戰場資料是 14×14 方格，社群說六角格；(2) `WAR.EXE` 裡唯一的數字鍵分派是 `sub_38DFE` 的**七項選單**，不是六方向——見 `docs/mechanics/30-combat.md` §3 |
