...
| **戰場幾何（M3）** | ✅ **confirmed**：**六角格**，`col=cell%14`／`row=cell/14`，`x=col×32`、`y=row×24+(奇數欄 +12)`。整張 448×348 塞進 BGI 640×350。單位記錄 `+5` = 格編號。見 `docs/re/07` |
...
| 1i | **`sub_55CEC` 通行規則** | 部署與移動共用的「這格站不站得下」判斷。解開它才能寫移動；也是六角鄰接規則最可能的所在 |
