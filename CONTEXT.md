...
| **政略規則（M3）** | 🔶 **起步**：指令 1=調動、6=查閱（其餘未確定）、命令數上限、省編號 = `3.15` 索引+1、13 個省份欄位、鄰接表樣本。見 `docs/playtest/02` |
...
| 1d | 省份鄰接表 | 攻打子選單會列出鄰省（河南 19 → 11,16,18,20,21,22）。`NWMAP.DAT`(15,360 B) 很可能就是這份資料，從檔案解比逐省實測快 |
